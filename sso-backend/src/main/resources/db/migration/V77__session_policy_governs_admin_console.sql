...
ALTER TABLE admin_portal_settings
    ...
    -- ON DELETE SET NULL: deleting the selected policy falls back to the acting admin's resolved policy
    -- rather than locking every admin out of the console.
    ADD COLUMN session_policy_id uuid REFERENCES session_policy (id) ON DELETE SET NULL;
