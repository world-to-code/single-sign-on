 ...
 * roll-up of their security factors (email verification, phone/SMS, TOTP, FIDO2 passkeys) and roles.
 */
public record ProfileView(String username, String email, String displayName, boolean emailVerified,
                          String phoneNumber, boolean phoneVerified, boolean totpEnrolled, boolean fido2Enrolled,
                          int passkeyCount, List<String> roles) {
    ...
    public static ProfileView of(UserAccount user, boolean totpEnrolled, int passkeyCount) {
        ...
        return new ProfileView(user.getUsername(), user.getEmail(), user.getDisplayName(), user.isEmailVerified(),
                user.getPhoneNumber(), user.isPhoneVerified(), totpEnrolled, passkeyCount > 0, passkeyCount, roles);
    }
}
