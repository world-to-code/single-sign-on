...
public class Fido2FactorHandler implements FactorHandler {
    ...
    @Override
    public FactorChallenge prepare(AppUser user, HttpServletRequest request) {
    ...
    private boolean register(AppUser user, PublicKeyCredentialCreationOptions options,
    ...
    private boolean assertCredential(AppUser user, PublicKeyCredentialRequestOptions options,
                                     FactorVerificationRequest verification, HttpSession session) {
        try {
