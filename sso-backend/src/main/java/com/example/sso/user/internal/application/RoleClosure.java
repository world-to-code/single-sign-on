...
class RoleClosure {
    ...
    /** The DIRECT parents of a role (the roles that inherit it directly), not the transitive ancestors. */
    Set<UUID> parentsOf(UUID childRoleId) {
        return new HashSet<>(edges.findParentRoleIdsByChildRoleId(childRoleId));
    }
}
