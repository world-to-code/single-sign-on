...
public class IntegrationTestSeeder implements ApplicationRunner {
    ...
    private void seedSimpleSamlPhpSp() {
        if (relyingParties.existsByEntityId(samlSpEntityId)) {
            return;
        }
        // Defaults: allows IdP-initiated SSO and does NOT require signed AuthnRequests, so a vanilla
        // SimpleSAMLphp SP works with no extra signing setup.
        relyingParties.save(new SamlRelyingParty(samlSpEntityId, samlSpAcsUrl, SamlRelyingParty.NAMEID_EMAIL));
        log.info("[integration-test] seeded SAML relying party '{}' (ACS {})", samlSpEntityId, samlSpAcsUrl);
    }
}
