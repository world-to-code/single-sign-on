...
/**
 * Encrypts/decrypts secrets stored at rest. New values use authenticated AES-256-GCM
 * ({@code encg:} prefix). For a gradual migration the reader also accepts legacy
 * AES-256-CBC values ({@code enc:}) and bare plaintext; both are upgraded to GCM the next
 * time the owning value is written (e.g. on key rotation or re-enrollment).
 */
@Component
public class SecretCipher {
    ...
    public SecretCipher(@Value("${sso.crypto.master-password:dev-master-password-change-me}") String masterPassword,
