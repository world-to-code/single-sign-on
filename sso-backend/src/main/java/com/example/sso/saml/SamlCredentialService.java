...
/**
 * Provides the IdP's SAML signing credential (RSA private key + self-signed X.509
 * certificate), persisted in a PKCS#12 keystore so issued assertions remain verifiable
 * across restarts. The keystore is generated on first start if absent.
 *
 * <p>OpenSAML-agnostic on purpose: it exposes plain JCA types; the OpenSAML
 * {@code BasicX509Credential} is assembled in the SAML configuration.
 */
@Service
public class SamlCredentialService {

    private static final Logger log = LoggerFactory.getLogger(SamlCredentialService.class);
    ...
    /**
     * Generates a new RSA signing keypair, persists it (replacing the keystore), and swaps it in
     * for subsequent assertions/metadata. Returns the new certificate's serial as a key id. SPs
     * must re-fetch IdP metadata to pick up the new certificate.
     */
