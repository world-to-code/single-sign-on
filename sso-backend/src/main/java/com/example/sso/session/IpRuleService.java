...
/**
 * CRUD + evaluation of IP access rules. The active rule set is compiled to {@link IpAddressMatcher}s
 * and cached so {@link #isAllowed} is cheap on every request. Semantics (evaluated by priority):
 * a matching enabled BLOCK rule denies; if any enabled ALLOW rule exists, the client must match one
