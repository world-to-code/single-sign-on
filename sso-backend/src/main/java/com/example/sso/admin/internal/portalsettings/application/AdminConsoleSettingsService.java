...
/**
 * Applies the admin console's settings — its governing session policy AND its console-only enforcement config
 * (elevation-token lifetime + entry IP allowlist) — in ONE transaction. A validation failure on either half
 * (e.g. a malformed CIDR) therefore rolls back both, so the console can never be left with a new session
 * policy but a stale/absent allowlist (a half-applied, fail-open network control).
 */
@Service
