    ...
    <>
      <section className="relative overflow-hidden border-b">
        <div className="relative mx-auto max-w-3xl px-4 py-20 text-center sm:px-6 sm:py-24">
          ...
          <p className="mx-auto mt-5 max-w-xl text-pretty text-lg text-muted-foreground">
            Register an OIDC client or SAML relying party, hand your app the discovery URL, and you're done.
            Keep users in sync automatically with SCIM 2.0 — joiners, movers, and leavers, handled for you.
          </p>
          <div className="mt-8">
            <Button size="lg" onClick={getStarted}>Connect your first app</Button>
          </div>
        </div>
      </section>
      ...
      <Section>
        <div className="mb-12 max-w-2xl">
          <Badge variant="muted" className="mb-3">Protocols</Badge>
          <h2 className="text-balance text-3xl font-semibold tracking-tight sm:text-4xl">
            Speak the standards your apps already know
          </h2>
          <p className="mt-3 text-muted-foreground">
            No proprietary SDK. Point your app at these endpoints and the flow just works.
          </p>
        </div>
      ...
      <Section tone="muted">
        <div className="mb-10 max-w-2xl">
          <Badge variant="muted" className="mb-3">Connection flow</Badge>
          <h2 className="text-balance text-3xl font-semibold tracking-tight sm:text-4xl">
            Three moves to a live integration
          </h2>
        </div>
      ...
      <Section>
        <div className="mb-8 max-w-2xl">
          <Badge variant="muted" className="mb-3">App catalog</Badge>
          <h2 className="text-balance text-3xl font-semibold tracking-tight sm:text-4xl">
            Works with the tools your team already uses
          </h2>
          <p className="mt-3 text-muted-foreground">
            Any OIDC- or SAML-compatible application connects. A few of the usual suspects:
          </p>
        </div>
        ...
        <p className="mt-6 text-sm text-muted-foreground">
          …and any other app that speaks OpenID Connect or SAML 2.0.
        </p>
      </Section>
...
const PROTOCOLS: {
  icon: LucideIcon; name: string; tag: string; body: string; chips: string[];
  code: { title: string; lines: Line[] };
}[] = [
  {
    icon: Plug, name: "OpenID Connect", tag: "OAuth 2.1-based",
    body: "Modern SSO with discovery, PKCE, and standard claims. Your app reads one URL and gets every endpoint and key it needs.",
  ...
  {
    icon: Network, name: "SAML 2.0", tag: "signed assertions",
    body: "Enterprise SSO for apps that only speak SAML, with signed assertions, IdP metadata, and single logout — each tenant on its own signing key.",
  ...
  {
    icon: RefreshCw, name: "SCIM 2.0", tag: "provisioning",
    body: "Automated user and group provisioning from your directory. Push a create, update, or delete and the change lands in real time under a scoped, tenant-bound token.",
...
const STEPS: { title: string; body: string }[] = [
  { title: "Register the app", body: "Add an OIDC client or SAML relying party in your admin console and copy its credentials." },
  { title: "Point it at discovery", body: "Give your app the OIDC discovery URL or SAML metadata — no custom code to write." },
  { title: "Sync and go live", body: "Optionally connect SCIM so users provision automatically, then ship." },
];
