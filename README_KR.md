...
테넌트는 `{slug}.base`(예: `acme.localhost`, `acme.idp.example.com`)에 산다. 인증 전에 호스트로 org를
정하기 때문에 다음이 성립한다.

- OIDC issuer가 테넌트마다 다르다(`http://acme.localhost`이 자기 discovery와 JWKS를 갖는다). 그 테넌트
...
```
mini-sso-system/
├── sso-backend/        Spring Boot IdP(Gradle 프로젝트). API 전용, 모든 auth/crypto/
│   ├── src/            protocol 로직을 소유. Dockerfile = API 이미지(SPA 번들 없음).
...
```bash
docker compose up -d                              # dev 인프라: PostgreSQL + Redis + MailHog
cd sso-backend && ./gradlew bootRun               # http://localhost:9000 에 API 전용 백엔드
# 다른 셸에서 — SPA dev 서버(Vite가 API/auth/OIDC/SAML을 :9000으로 프록시):
cd sso-frontend && npm install && npm run dev     # http://localhost:5173
```

http://localhost:5173 을 열고 `admin` / `admin123!`(플랫폼 super-admin)로 로그인한다. 새 사용자는 최초
로그인 때 이메일 코드(MailHog http://localhost:8025 에서 확인)와 강한 factor(QR로 TOTP, 또는 passkey)를 거친다.

**분리 배포를 로컬에서** 실행(nginx 에지 + API 전용 백엔드 — 프로덕션 토폴로지):

```bash
docker compose -f docker-compose.prod.yml up --build    # 단일 공개 오리진 http://localhost
```

테넌트는 서브도메인에 있다. 대부분 시스템에서 `*.localhost`가 `127.0.0.1`로 해석되므로, 분리 스택에서 slug가
`acme`인 테넌트는 `http://acme.localhost`에서 접근한다 — 에지가 전체 `Host`를 그대로 넘겨 백엔드가 테넌트와
그 per-tenant issuer를 유도한다.(Vite dev 서버는 호스트를 플랫폼 오리진으로 바꾸므로, 서브도메인 테넌시는
에지로 확인한다.) 관리자 콘솔(Organizations)이나 공개 셀프서비스 가입으로 만들 수 있고, 활성화 링크는 플랫폼
호스트로 도착한 뒤 사용을 마치면 새 관리자를 자기 서브도메인으로 보낸다.
...
**분리 배포.** 이미지는 서비스별 Dockerfile로 각각 빌드한다 — API 백엔드
(`docker build -t mini-sso-backend sso-backend/`)와, SPA를 서빙하고 백엔드로 리버스 프록시하는 nginx 에지
(`docker build -t mini-sso-frontend sso-frontend/`). `docker-compose.prod.yml`이 전체 스택을 배선한다. 두
런타임 이미지는 **Alpine + non-root**다. 세션은 **Redis**에 있어 백엔드가 에지 뒤에서 수평 확장되고(sticky
세션 불필요), 리보케이션은 Redis keyspace 이벤트로 노드 간에 전파된다.

**시크릿 (SOPS).** `docker-compose.prod.yml`은 모든 시크릿을 **필수**(`${VAR:?…}`)로 받아, 약한 기본값으로
뜨는 것을 거부한다(`admin123!` fail-open 없음). 값은 [SOPS](https://github.com/getsops/sops) +
[age](https://github.com/FiloSottile/age)로 **암호문만 커밋 안전**하게 만든 파일에서 공급한다(시크릿 *값*만
암호화; 개인키는 절대 커밋 안 함). 최초 설정: `age-keygen`으로 키 생성 → 공개키를 `.sops.yaml`에 붙여넣기 →
`cp secrets/prod.example.yaml secrets/prod.sops.yaml`에 실제 값 채운 뒤 `sops --encrypt --in-place`. 실행은
시크릿을 프로세스 환경으로 복호화해 넘긴다(디스크에 안 씀):
`SOPS_AGE_KEY_FILE=age.key sops exec-env secrets/prod.sops.yaml 'docker compose -f docker-compose.prod.yml up --build'`.
